...
import { z } from 'zod';
import { CREATED, SUCCESS, BAD_REQUEST, UNAUTHORIZED, SERVER_ERROR, MISSING_BODY_FIELDS } from '../../utils/httpCodeResponses/messages';
...
export const register = async (req: Request, res: Response) => {
	try {
		...
		if (userExists) return BAD_REQUEST(res, 'Użytkownik o tym adresie email już istnieje.');
		...
		return CREATED(res, 'Użytkownik został pomyślnie zarejestrowany.', {
			token,
			user: { id: newUser.id, email: newUser.email, name: newUser.name, isSuperadmin: newUser.isSuperadmin },
		});
	} catch {
...
export const login = async (req: Request, res: Response) => {
	try {
		...
		const isPasswordValid = await authService.verifyPassword(parsed.data.password, user.password);
		if (!isPasswordValid) return UNAUTHORIZED(res, 'Nieprawidłowy email lub hasło.');
